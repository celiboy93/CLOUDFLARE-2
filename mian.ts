// main.ts (v1.29 - Final Code with R2 Fixes & Telegram Link Generator)
import {
  S3Client,
  PutObjectCommand,
...
const R2_PUBLIC_URL = Deno.env.get("R2_PUBLIC_URL")!; // **NOTE: MUST NOT contain https:// or trailing / **
...
// NEW: Function to get the full Telegram file link
async function getFileLink(fileId: string): Promise<string | null> {
  if (!TELEGRAM_BOT_TOKEN) return null;
  
  // 1. Call getFile method to get the file_path
  const apiURL = `https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/getFile?file_id=${fileId}`;
  ...
  try {
    ...
    if (data.ok && data.result && data.result.file_path) {
      ...
      const directLink = `https://api.telegram.org/file/bot${TELEGRAM_BOT_TOKEN}/${filePath}`;
      return directLink;
    }
  } catch (e) {
...
Deno.serve(async (req: Request) => {
        ...
        <script>
          ...
          function setResult(data, type = 'success') {
            if (type === 'error') {
              ...
              // NOTE: R2 Link will be displayed with double https:// if R2_PUBLIC_URL has https:// in its value.
              resultDiv.innerHTML = \`
  ...
  if (req.method === "POST" && url.pathname === "/upload-file") {
    try {
      ...
      const r2Link = `https://${R2_PUBLIC_URL}/${fileName}`; // R2_PUBLIC_URL MUST NOT contain https://
  ...
  // --- 6H. Handle Telegram Link Generation (NEW ROUTE) ---
  if (req.method === "GET" && url.pathname === "/telegram-link") {
    const fileId = url.searchParams.get("file_id");
    
    if (!fileId) {
      return new Response(JSON.stringify({ error: "Missing file_id parameter. Use /telegram-link?file_id=<ID>" }), {
        status: 400,
        headers: { "Content-Type": "application/json" }
      });
    }
    ...
    if (!TELEGRAM_BOT_TOKEN) {
      return new Response(JSON.stringify({ error: "TELEGRAM_BOT_TOKEN is not set in environment variables." }), {
        status: 500,
        headers: { "Content-Type": "application/json" }
      });
    }

    const directLink = await getFileLink(fileId);

    if (directLink) {
      return new Response(JSON.stringify({ 
        fileId: fileId,
        directLink: directLink,
        note: "This link is temporary and may expire after a few hours or days."
      }), {
        status: 200,
        headers: { "Content-Type": "application/json" }
      });
    } else {
      return new Response(JSON.stringify({ error: "Failed to get file link from Telegram API. Check your file_id or bot token." }), {
        status: 500,
        headers: { "Content-Type": "application/json" }
      });
    }
