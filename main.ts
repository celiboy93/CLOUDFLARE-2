...
for (const varName of REQUIRED_ENV_VARS) {
  if (!Deno.env.get(varName)) {
    ...
    // Deno Deploy does not allow Deno.exit(), so we only throw the error.
        ...
        <script>
          ...
          function setResult(data, type = 'success') {
            if (type === 'error') {
              ...
              // FIX: Ensure data.proxyUrl is correctly displayed as a JS variable
              resultDiv.innerHTML = \`
